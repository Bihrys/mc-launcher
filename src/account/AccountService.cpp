...
QString AccountService::offlineAvatarPreview(const QString &username) const {
    return "https://crafatar.com/avatars/" + offlineUuid(username.trimmed()) + "?overlay";
}
...
QJsonObject AccountService::publicPayload(const QJsonArray &accounts) const {
    ...
    for (int i = 0; i < accounts.size(); ++i) {
        ...
        out.append(QJsonObject{
            ...
            {"serverUrl", server}, {"avatarUrl", a.value("avatarUrl").toString(offlineAvatarPreview(username))},
...
QJsonObject AccountService::addOffline(const QString &username) {
    ...
    accounts.append(QJsonObject{{"kind", "offline"}, {"username", name}, {"uuid", uuid}, {"accessToken", "0"}, {"selected", true}, {"avatarUrl", offlineAvatarPreview(name)}, {"note", "离线账户"}});
...
QJsonObject AccountService::addYggdrasilPlaceholder(const QString &serverUrl, const QString &username) {
    ...
    accounts.append(QJsonObject{{"kind", "yggdrasil"}, {"username", name}, {"uuid", uuid}, {"serverUrl", serverUrl.trimmed()}, {"selected", true}, {"avatarUrl", offlineAvatarPreview(name)}, {"note", "C++ 重构骨架：第三方登录接口占位，后续按 HMCL authlib-injector 流程接入"}});
    saveAccounts(accounts);
    return publicPayload(accounts);
}
...
QJsonObject AccountService::addMicrosoftPlaceholder(const QString &clientId) {
    ...
    accounts.append(QJsonObject{{"kind", "microsoft"}, {"username", name}, {"uuid", uuid}, {"selected", true}, {"avatarUrl", offlineAvatarPreview(name)}, {"note", "C++ 重构骨架：Microsoft OAuth 后续按 HMCL MicrosoftAccountFactory 接入"}});
    saveAccounts(accounts);
    return publicPayload(accounts);
}
