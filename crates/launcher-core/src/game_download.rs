...
fn install_fabric_or_quilt(
    ...
    manager.set_message(format!("正在获取 {loader_kind} profile..."))?;
    ...
    let libraries = collect_libraries_from_version_json(source, &root, &profile_json)?;
...
fn download_loader_installer(
    ...
    manager.set_message(format!("正在查找 {loader_kind} installer..."))?;
...
fn install_version_json(
        ...
        .and_then(|value| value.get("client"))
    {
        if let Some(url) = client_download.get("url").and_then(Value::as_str) {
            ...
            files.push(download_file_from_artifact(
                source,
                url,
                &jar_path,
                client_download,
...
fn collect_libraries_from_version_json(
    source: DownloadSource,
    root: &Path,
    version_json: &Value,
) -> Result<Vec<DownloadFile>, DownloadError> {
    ...
    for lib in libraries {
        if let Some(artifact) = lib.get("downloads").and_then(|value| value.get("artifact")) {
            ...
            if let Some(file) = library_from_name_to_file(source, root, base_url, name) {
                files.push(file);
            }
        }
        ...
        {
            if let Some(file) = library_artifact_to_file(source, root, classifier) {
                files.push(file);
            }
        }
    }
...
fn library_artifact_to_file(
    ...
    Some(download_file_from_artifact(
        source,
        url,
...
fn library_from_name_to_file(
    source: DownloadSource,
    root: &Path,
    base_url: &str,
    descriptor: &str,
) -> Option<DownloadFile> {
    let path = maven_path(descriptor)?;
    ...
    Some(DownloadFile::with_candidates(
        inject_url_candidates(source, &(base_url + &path)),
        root.join("libraries").join(path),
        None,
        None,
    ))
...
fn collect_assets_from_version_json(
    manager: &DownloadManager,
    source: DownloadSource,
    client: &Client,
    root: &Path,
    ...
    for object in objects.values() {
        ...
        files.push(DownloadFile::with_candidates(
            asset_object_candidates(source, prefix, hash),
            target,
            size,
            Some(hash.to_string()),
        ));
...
fn download_file_from_artifact(
    source: DownloadSource,
    url: &str,
    target: &Path,
    artifact: &Value,
    ...
    DownloadFile::with_candidates(
        inject_url_candidates(source, url),
        target.to_path_buf(),
...
fn inject_url_candidates(source: DownloadSource, url: &str) -> Vec<String> {
    match source {
        DownloadSource::Official => vec![url.to_string()],
        DownloadSource::Bmcl | DownloadSource::Mirror => unique_urls(vec![inject_bmcl_url(url)]),
        DownloadSource::Balanced => unique_urls(vec![inject_bmcl_url(url), url.to_string()]),
    }
}

fn asset_object_candidates(source: DownloadSource, prefix: &str, hash: &str) -> Vec<String> {
    ...
    match source {
        DownloadSource::Official => vec![official],
        DownloadSource::Bmcl | DownloadSource::Mirror => vec![bmcl],
        DownloadSource::Balanced => unique_urls(vec![bmcl, official]),
    }
}
...
fn parse_source(source: &str) -> Result<DownloadSource, DownloadError> {
    match source.trim().to_ascii_lowercase().as_str() {
        "" | "auto" | "balanced" => Ok(DownloadSource::Balanced),
        "official" | "mojang" => Ok(DownloadSource::Official),
        "mirror" => Ok(DownloadSource::Mirror),
...
impl DownloadSource {
    fn as_str(self) -> &'static str {
        match self {
            ...
            DownloadSource::Balanced => "balanced",
